...
class _SayimEkraniState extends State<SayimEkrani> {
  ...
  // Taratılmış barkodlar için bir set oluşturduk
  ...
  Future<void> _fetchEnvanterSayisi() async {
    ...
    try {
      final totalResponse = await http.get(Uri.parse(
        isLocationSelected
            ? 'http://10.10.208.135:8083/api/materials/total/$selectedRoomId'
            : 'http://10.10.208.135:8083/api/materials/total/${selectedPersonelId!}/$selectedRoomId',
      ));
      final totalData = json.decode(totalResponse.body);

      final foundResponse = await http.get(Uri.parse(
        isLocationSelected
            ? 'http://10.10.208.135:8083/api/materials/found/$selectedRoomId'
            : 'http://10.10.208.135:8083/api/materials/found/${selectedPersonelId!}/$selectedRoomId',
      ));
      final foundData = json.decode(foundResponse.body);

      if (!isLocationSelected) {
        final otherLocationsResponse = await http.get(Uri.parse(
          'http://10.10.208.135:8083/api/materials/other-locations/${selectedPersonelId!}/$selectedRoomId',
        ));
  ...
  void _checkBarcode() async {
    if (scannedBarcode != null && scannedBarcode!.isNotEmpty) {
      // Barkod daha önce tarandı mı kontrolü
      ...
      // Barkod yeni, set'e ekle
      scannedBarcodes.add(scannedBarcode!);

      try {
        final url = isLocationSelected
            ? 'http://10.10.208.135:8083/api/materials/update-status?barkodNo=$scannedBarcode&found=true'
            : 'http://10.10.208.135:8083/api/personel/update-material-status';
    ...
    if (!isLocationSelected &&
      ...
      try {
        final materialsResponse = await http.get(Uri.parse(
            'http://10.10.208.135:8083/api/personel/materials?perId=$selectedPersonelId'));
        ...
        for (var material in foundMaterials) {
          ...
          final url = 'http://10.10.208.135:8083/api/found-materials/save';
