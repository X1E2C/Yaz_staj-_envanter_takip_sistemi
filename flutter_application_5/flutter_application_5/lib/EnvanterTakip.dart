  ...
  Future<void> _fetchTotalMaterialCount() async {
    ...
    try {
      final locationIdResponse = await http.get(Uri.parse(
          'http://10.10.208.135:8083/api/location/${widget.username}'));
      final locationId = json.decode(locationIdResponse.body);

      final String apiUrl = widget.isPersonelBazli
          ? 'http://10.10.208.135:8083/api/materials/personel-total/$locationId'
          : 'http://10.10.208.135:8083/api/materials/location/$locationId/total';
    ...
    int? pickedYear = await showDialog<int>(
      ...
      builder: (BuildContext context) {
        return YearPickerDialog(
          ...
          minYear: 2023,
          maxYear: DateTime.now().year, // Son yıl dinamik olarak ayarlandı
        );
      },
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: <Widget>[
            const SizedBox(height: 10), // Yıl seç butonu biraz yukarıda olsun
            ElevatedButton(
              onPressed: () => _selectYear(context),
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color.fromARGB(
                    255, 204, 44, 44), // Diğer butonlarla aynı renk
                padding:
            ...
            const SizedBox(height: 20), // Boşluk ile üst ve alt butonları ayır
            Container(
